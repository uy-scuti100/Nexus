...
export default function PopUpProvider({ children }: { children: ReactNode }) {
   const { openModal, toggleFormModal, toggleJoinFormModal } = useContext(
      ModalContext
   ) as ModalContextProp;
   return (
      <div className="fixed inset-0 z-50 w-full h-full overflow-hidden transition-all duration-300">
         ...
         <div className="absolute flex justify-center items-center z-50 top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%]">
            <div className="absolute right-0 p-2 top-[-60px] cursor-pointer ">
               <X onClick={openModal ? toggleFormModal : toggleJoinFormModal} />
            </div>
            {children}
         </div>
      </div>
